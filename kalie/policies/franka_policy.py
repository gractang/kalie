...
from kalie.utils.utils import print_green
...
class FrankaMarkPolicy(object):
    ...
    def __init__(self,
                 # config_path='configs/franka_mark_policy.yaml',
                 config,
                 task,
                 fix_angle,
                 pre_contact_height,
        ...
        self.pre_contact_height = pre_contact_height
        self.post_contact_height = post_contact_height
    ...
    def reset(self, observation, context, init=False):  # NOQA
        ...
        if self.grasp_point is None:
            ...
            # TODO(fangchen): This causes an ambiguous orientation issue.
            grasp_yaw = (context['grasp_yaw']) % (np.pi) - np.pi / 2
            # grasp_yaw = 0

            if self.fix_angle:
                grasp_euler = np.array([np.pi, 0, 0])
            else:
        ...
        # context['target_euler'] = 'forward'  # TODO: Debugging
        # target_euler = np.array([np.pi, 0, 0])
        
        context['target_euler'] = self.target_euler
        if 'target_euler' not in context or context['target_euler'] is None:
            # TODO (fangchen): Get the target euler angles from the planner.
            target_euler = np.array([np.pi, 0, grasp_yaw])
        else:
            # TODO (fangchen): Double check if these are correct.
            if context['target_euler'] == 'forward':
                target_euler = np.array([np.pi, 0, 0])
            elif context['target_euler'] == 'backward':
                target_euler = np.array([np.pi, 0, np.pi])
            elif context['target_euler'] == 'left':
                target_euler = np.array([np.pi, 0, np.pi / 2])
            elif context['target_euler'] == 'right':
                target_euler = np.array([np.pi, 0, -np.pi / 2])
            elif context['target_euler'] == 'upward':
                target_euler = np.array([np.pi, -np.pi / 6, 0])
            elif context['target_euler'] == 'downward':
                # target_euler = np.array([-np.pi / 2, 0, 0]) 
                target_euler = np.array([np.pi, np.pi / 6, 0]) 
            else:
                raise ValueError
        ...
        # Task specific: ignore certain points
        if self.task == "drawer_closing":
            # No grasp or function point
            self.grasp_point = None
            self.function_point = None
        elif self.task == "towel_hanging":
            # No function point
            self.function_point = None

        if self.task == "unplugging" or self.task == "pouring": # tasks missing either pre or post contact points
            if self.task == "unplugging":
                # No function or waypoints
                self.function_point = None
                self.pre_contact_waypoints = []
            elif self.task == "pouring":
                # No post contact, pre-contact is assigned
                self.pre_contact_waypoints = context['waypoints_3d']['pre_contact']
                
            self.post_contact_waypoints = []
        ...
        print_green(self.post_contact_height)

        self.phases = []

        # lift_open: rise up with gripper open
        # grip: close gripper
        # reach_pre_grasp: go above grasp point (to avoid collisions)
        # reach_grasp: go down to grasp point
        # lift: lift, but with gripper closed? or maybe in same position?
        if not init:
            self.phases += [
                'lift_open',
            ]
        ...
        if self.grasp_point is None:
            ...
            if not (self.target_point is None and self.function_point is None):
                self.phases += [
                    'lift',
                ]
            if self.task == "unplugging":
                self.phases.remove('lift')
    ...
    def sample_actions(  # NOQA
        ...
        if current_phase == 'release':
            ...
            action = self.open_gripper(goal_pose) # temp comment out, annoying to keep having to pick items up
            ...
            euler = self.grasp_pose[3:6].copy()
            ...
            if self.pre_contact_height == 'same':
                ...
                pos[2] = self.config.motion.safe_z
                vel = self.config.motion.high_vel
            elif self.pre_contact_height == 'mid':
                pos[2] = self.config.motion.safe_z - 0.15
                vel = self.config.motion.high_vel
            else:
            ...
            if self.target_height is None:
                pass
            elif self.target_height == 'above':
                pos[2] = self.config.motion.safe_z
                vel = self.config.motion.high_vel
            elif self.target_height == 'same':
                if self.grasp_height is not None:
                    # print_green(pos[2])
                    pos[2] = max(pos[2], self.grasp_height)
                vel = self.config.motion.low_vel
            ...
            if self.post_contact_height == 'same':
                ...
                pos[2] = self.config.motion.safe_z
                vel = self.config.motion.high_vel
            elif self.post_contact_height == 'mid':
                pos[2] = self.config.motion.safe_z - 0.15
                vel = self.config.motion.high_vel
            else:
                raise ValueError('Unrecognized height: %s'
    ...
    def move_to(self, target_pose, gripper_pose, grip, vel,
        ...
        # target_euler = np.array(target_pose[3:6], dtype=np.float32)  # TODO
        ...
        if self.debug:
            ...
            print('delta_target_pose: ', target_pose - gripper_pose)
            print('delta_pos: ', d_pos, 'delta_euler: ', d_euler,
                  'pos_done: ', pos_done, 'euler_done: ', euler_done)
