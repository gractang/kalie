...
def prepare_dino(home_dir='/home/grace/GroundingDINO'):
    HOME = home_dir
...
def get_object_bboxes(image_torch, texts):
    ...
    # print(torch.stack(all_boxes, dim=0).shape, torch.stack(all_logits, dim=0).shape, all_phrases)
    # print(boxes.shape, logits.shape, phrases)
...
def get_segmentation_masks(image, texts, boxes, logits, phrases, sam_checkpoint, visualize=False, logdir=None):
    image_np, image_torch = load_pil_image(image)
    # # get bounding box
    # boxes, logits, phrases = get_object_bboxes(image_torch, texts)
    #
