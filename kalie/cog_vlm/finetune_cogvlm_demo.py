...
from utils.cog_vlm_models import FineTuneTrainCogVLMModel
...
"""
Creates the dataset function via loading the relevant json file into data dictionary:
- data['img'] is the relative path to the image, will also represent the uni_key 
- data['points'] is the label we input into the text_processor, and should be of the form
    "Grasp Point: (a, b), Function Point: (c, d), Target Point: (e, f), 
    Pre-contact Point: (g, h), Post-contact Point: (i, j)"
- data['other']['task'] is the task that we format into our task prompt template, 
    which then forms the final prompt
- data['text_dict'] represents output of the call to text_processor with the label + task prompt 
    (? currently unused)
- assert "labels" in d['text_dict'] - somehow the call to text_processor needs to add "labels" 
    (? currently unused) 
"""
def create_dataset_function(image_processor, text_processor, path, args):
    ...
    with open(path, "r") as fb:  # have a separate path for training, validation, and testing
        data = json.load(fb)
    if path == file_path_config["TRAIN_PATH"]: 
        dataset = AgentItemDataset(image_processor, text_processor, list(data.values()), True)
    else: 
...
if __name__ == '__main__':
    ...
    if args.use_lora:
        ...
        args.use_lora = False
        args.save = "checkpoints/merged_lora_cogvlm_{}_{}".format(args.eva_args["image_size"][0], datetime.now().strftime('%Y-%m-%d-%H-%M'))
