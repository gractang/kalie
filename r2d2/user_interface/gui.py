...
class RobotGUI(tk.Tk):
    ...
    def robot_reset(self, event):
        ...
        if self.enter_presses == 25:
            ...
            self.frames[RobotResetPage].set_home_frame(
                type(self.curr_frame)
            )
...
class LoginPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Title #
        title_lbl = Label(
            self, text="Login Page", font=Font(size=30, weight="bold")
        )
        ...
        instr_lbl = Label(
            self, textvariable=self.warning_text, font=Font(size=20)
        )
        ...
        name_lbl = Label(
            self, text="Full Name:", font=Font(size=15, underline=True)
        )
        name_lbl.place(relx=0.5, rely=0.22, anchor="n")
        self.name_entry = tk.Entry(
            self, textvariable=self.user, width=35, font=Font(size=15)
        )
        ...
        building_lbl = Label(
            self, text="Building:", font=Font(size=15, underline=True)
        )
        building_lbl.place(relx=0.5, rely=0.32, anchor="n")
        self.building_entry = tk.Entry(
            self, textvariable=self.building, width=35, font=Font(size=15)
        )
        self.building_entry.place(relx=0.5, rely=0.35, anchor="n")

        # New Scene Button #
        scene_change_lbl = Label(
            self,
            text="Has The Scene Changed?",
            font=Font(size=15, underline=True),
        )
    ...
    def check_completeness(self):
        ...
        name_correct = (name_num_words >= 2) and (
            missing_name_text not in name
        )
        ...
        if not name_correct:
            ...
            update_gui_info(
                user=name, building=building, scene_id=self.curr_scene_id
            )
...
class RobotResetPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        title_lbl = Label(
            self,
            text="Resetting Robot...",
            font=Font(size=30, weight="bold"),
        )
        title_lbl.pack(pady=15)

        description_lbl = Label(
            self, text="Please stand by :)", font=Font(size=18)
        )
        description_lbl.pack(pady=5)

    def launch_page(self):
        print('reset at robot reset page!')
...
class CanRobotResetPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        title_lbl = Label(
            self,
            text="Proceed With Robot Reset?",
            font=Font(size=30, weight="bold"),
        )
        title_lbl.pack(pady=15)

        description_lbl = Label(
            self, text="Press 'A' when ready", font=Font(size=18)
        )
        description_lbl.pack(pady=5)
    ...
    def moniter_keys(self, event):
        if self.controller.curr_frame != self:
            return
        self.controller.frames[RobotResetPage].set_home_frame(
            self.next_page
        )
...
class ControllerOffPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.controller = controller
        self.controller.bind(
            "<KeyRelease-space>", self.moniter_keys, add="+"
        )

        title_lbl = Label(
            self,
            text="WARNING: Controller off",
            font=Font(size=30, weight="bold"),
        )
        title_lbl.pack(pady=15)

        description_lbl = Label(
            self, text=controller_off_msg, font=Font(size=18)
        )
        description_lbl.pack(pady=5)
...
class CalibrationPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Title #
        how_to_title_lbl = Label(
            self, text="Calibration Hub", font=Font(size=30, weight="bold")
        )
        ...
        instr_lbl = tk.Label(
            self,
            text=color_spectrum_explantion,
            font=Font(size=20, slant="italic"),
        )
        instr_lbl.pack(pady=5)

        # How To Text #
        how_to_text_lbl = Label(
            self, text=how_to_calibrate_text, font=Font(size=18)
        )
        how_to_text_lbl.pack(pady=20)

        longest_name = max(
            [len(get_camera_name(cam_id)) for cam_id in controller.cam_ids]
        )
        ...
        for i in range(len(controller.cam_ids)):
            ...
            camera_btn = tk.Button(
                ...
                font=Font(size=30, weight="bold"),
                width=longest_name,
                command=lambda cam_idx=cam_id: self.calibrate_camera(
                    cam_idx
                ),
                borderwidth=10,
            )
    ...
    def initialize_page(self):
        ...
        for cam_id in self.button_dict.keys():
            is_missing = any(
                [
                    cam_id in missing_id
                    for missing_id in info_dict["missing"]
                ]
            )
    ...
    def exit_page(self):
        if (
            self.controller.curr_frame
            != self.controller.frames[CalibrateCamera]
        ):
...
class OldScene(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Title #
        how_to_title_lbl = Label(
            self, text="Scene Warning", font=Font(size=30, weight="bold")
        )
        how_to_title_lbl.pack(pady=5)

        # Warning #
        instr_lbl = tk.Label(
            self, text=old_scene_text, font=Font(size=20, slant="italic")
        )
        instr_lbl.pack(pady=5)
...
class PreferredTasksPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # Title #
        title_lbl = Label(
            self, text="Preferred Tasks", font=Font(size=30, weight="bold")
        )
        title_lbl.place(relx=0.5, rely=0.05, anchor="n")
        ...
        for key in preferred_tasks.keys():
            ...
            for i, task in enumerate(preferred_tasks[key]):
                ...
                task_ckbox.place(
                    relx=x_pos + 0.01, rely=y_pos + (i + 1) * 0.04
                )
        ...
        notes_lbl = tk.Label(
            self,
            text="Personal Notes:",
            font=Font(size=20, underline=True),
        )
        notes_lbl.place(relx=0.05, rely=0.6)

        self.notes_txt = tk.Text(
            self, height=15, width=65, font=Font(size=15)
        )
    ...
    def moniter_keys(self, event):
        ...
        if event.keysym in ["Shift_L", "Shift_R"]:
            self.controller.frames[CameraPage].set_home_frame(
                PreferredTasksPage
            )
...
class SceneConfigurationPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.controller.bind("<KeyRelease>", self.moniter_keys, add="+")
        self.controller.bind(
            "<ButtonRelease-1>", self.moniter_keys, add="+"
        )
        ...
        box_lbl = tk.Button(
            ...
            text=" " * 25,
            highlightbackground="blue",
            font=Font(slant="italic", weight="bold"),
            padx=12,
            pady=40,
        )
        ...
        ideas_btn.bind(
            "<Button-1>", lambda e: webbrowser.open_new(task_ideas_link)
        )
        ...
        # Shift Instructions #
        instr_lbl = tk.Label(
            self, text=shift_text, font=Font(size=20, slant="italic")
        )
        instr_lbl.place(relx=0.5, rely=0.1, anchor="n")
        ...
        # Free Response Tasks #
        group_lbl = tk.Label(
            self, text=freewrite_text, font=Font(size=20, underline=True)
        )
        group_lbl.place(relx=0.01, rely=0.6)

        self.task_txt = tk.Text(
            self, height=15, width=65, font=Font(size=15)
        )
...
class RequestedBehaviorPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        task_lbl = Label(
            self, textvariable=self.task_text, font=Font(size=30)
        )
        ...
        instr_lbl = tk.Label(
            self,
            text="Press 'A' to begin, or 'B' to resample",
            font=Font(size=20, slant="italic"),
        )
        instr_lbl.place(relx=0.5, rely=0.1, anchor="n")
        ...
        bx, by = 0.15, 0.045
        box_lbl = tk.Button(
            self, text=" " * 48, highlightbackground="black", pady=26
        )
        box_lbl.place(relx=bx, rely=by, anchor="n")
        ...
        success_btn.bind(
            "<Button-1>", lambda e: self.change_trajectory_status(True)
        )
        ...
        failure_btn.bind(
            "<Button-1>", lambda e: self.change_trajectory_status(False)
        )
        ...
        # Update Based Off Activity #
        controller.bind(
            "<<KeyRelease-controllerA>>", self.start_trajectory, add="+"
        )
        controller.bind(
            "<<KeyRelease-controllerB>>", self.resample, add="+"
        )
    ...
    def sample_single_task(self):
        fixed_tasks = self.controller.info["fixed_tasks"]
        ft_weight = np.array(
            [self.get_task_weight(t) for t in fixed_tasks]
        )
        ft_weight = (ft_weight / ft_weight.sum()) * (1 - new_task_prob)

        new_tasks = self.controller.info["new_tasks"]
        nt_weight = (
            np.ones(len(new_tasks)) / len(new_tasks)
        ) * new_task_prob
...
class SceneChangesPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        instr_lbl.place(relx=0.5, rely=0.1, anchor="n")
        self.controller.bind(
            "<KeyRelease>", self.show_camera_feed, add="+"
        )

        # Changes #
        self.change_text = StringVar()
        change_lbl = Label(
            self, textvariable=self.change_text, font=Font(size=30)
        )
    ...
    def sample_change(self):
        ...
        move_robot = (num_traj % move_robot_frequency == 0) and (
            num_traj > 0
        )
...
class CameraPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        for i in range(self.n_rows):
            ...
            for j in range(self.n_cols):
                if (i * self.n_cols + j) >= len(
                    self.controller.camera_order
                ):
                    continue
                ...
                button = tk.Button(
                    ...
                    command=lambda idx=(
                        i * self.n_cols + j
                    ): self.update_image_grid(idx),
                )
                button.grid(
                    row=i, column=j, sticky="s" if self.n_rows > 1 else ""
                )
    ...
    def update_camera_feed(self, i, w_coeff=1.0, h_coeff=1.0):
        while True:
            ...
            w, h = max(self.winfo_width(), 100), max(
                self.winfo_height(), 100
            )
    ...
    def end_trajectory(self):
        ...
        scene_change = (
            np.random.uniform() < scene_change_prob
        ) or move_robot
...
class EnlargedImagePage(tk.Frame):
    ...
    def return_to_camera_grid(self, e):
        ...
        self.controller.show_frame(
            CameraPage, refresh_page=False, wait=True
        )
    ...
    def update_camera_feed(self):
        while True:
            ...
            if not_active or not_ready:
                time.sleep(0.05)
                continue
            w, h = max(self.winfo_width(), 250), max(
                self.winfo_height(), 250
            )
            self.controller.set_img(
                self.img_index, widget=self.image_box, width=w, height=h
            )
...
class CalibrateCamera(tk.Frame):
    def __init__(self, parent, controller, num_views=2):
        ...
        # Moniter Key Events #
        self.controller.bind(
            "<<KeyRelease-controllerA>>", self.press_A, add="+"
        )
        self.controller.bind(
            "<<KeyRelease-controllerB>>", self.press_B, add="+"
        )
    ...
    def collect_trajectory(self):
        ...
        self.instr_str.set(
            "Press 'A' to begin camera calibration, and 'B' to terminate early"
        )
        success = self.controller.robot.calibrate_camera(
            self.cam_id, reset_robot=False
        )
    ...
    def end_trajectory(self, success):
        ...
        if success:
            time.sleep(
                0.1
            )  # Prevents bug where robot doesnt wait to reset
            self.controller.frames[CanRobotResetPage].set_next_page(
                CalibrationPage
            )
            self.controller.show_frame(CanRobotResetPage)
            ...
            self.instr_str.set(
                "Press 'A' to retry, or 'B' to go back to the calibration hub"
            )
    ...
    def set_camera_id(self, cam_id):
        ...
        self.instr_str.set(
            "Press 'A' to begin, or 'B' to go back to the calibration hub"
        )
        self.back_btn.place(relx=0.5, rely=0.85, anchor="n")
        self.relevant_indices = []
        self.cam_id = cam_id
    ...
    def update_camera_feed(self, i, w_coeff=1.0, h_coeff=1.0):
        while True:
            ...
            self.controller.set_img(
                index,
                widget=self.image_boxes[i],
                width=img_w,
                height=img_h,
                use_camera_order=False,
            )
