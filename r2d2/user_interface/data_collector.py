...
class DataCollecter:
    ...
    def collect_trajectory(
        self, info=None, practice=False, reset_robot=True
    ):
        ...
        if practice or (not self.save_data):
            ...
            save_filepath = os.path.join(
                self.failure_logdir, info["time"], "trajectory.h5"
            )
            recording_folderpath = os.path.join(
                self.failure_logdir, info["time"], "recordings"
            )
        ...
        # Sort Trajectory #
        self.traj_saved = controller_info["success"] and (
            save_filepath is not None
        )
...
# class MouseClickDataCollecter(DataCollecter):
#     def __init__(
#         self,
#         env,
#         controller,
...
#         self.traj_running = True
#         self.env._robot.establish_connection()
#         controller_info = tu.collect_trajectory_with_mouseclick(
#             self.env,
#             metadata=info,
