...
def change_position_to_frame(position, frame, degrees=False):
    R_frame = euler_to_rmat(frame[3:6], degrees=degrees)
    t_frame = frame[:3]
    position_new = R_frame @ position + t_frame
    return position_new


def change_position_from_frame(position, frame, degrees=False):
    R_frame = euler_to_rmat(frame[3:6], degrees=degrees)
    t_frame = frame[:3]

    new_t_frame = np.dot(-t_frame, R_frame)
    new_R_frame = R_frame.T

    position_new = new_R_frame @ position + new_t_frame
    return position_new


def inverse_frame(frame, degrees=False):
    R_frame = euler_to_rmat(frame[3:6], degrees=degrees)
    t_frame = frame[:3]
    new_t_frame = np.dot(-t_frame, R_frame)
    new_R_frame = R_frame.T
    return np.concatenate([new_t_frame, rmat_to_euler(new_R_frame)], -1)
