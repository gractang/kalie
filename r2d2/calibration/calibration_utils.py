...
def load_calibration_info(keep_time=False):
    ...
    if not keep_time:
        calibration_info = {
            key: data["pose"] for key, data in calibration_info.items()
        }
...
def workspace_clip(pose, workspace=None):
    if workspace is None:
        return pose
    pose = np.copy(pose)
    pose[:3] = np.clip(pose[:3], workspace[0], workspace[1])
    return pose
...
class CharucoDetector:
    ...
    def calculate_target_to_cam(self, readings, train=True):
        ...
        rmats = [
            R.from_rotvec(rvec.flatten()).as_matrix() for rvec in rvecs
        ]
    ...
    def augment_image(
        ...
        assert all(
            [t in ["markers", "charuco", "axes"] for t in visual_type]
        )
        if image.shape[2] == 4:
        ...
        if "axes" in visual_type:
            ...
            cv2.drawFrameAxes(
                image, cameraMatrix, distCoeffs, rvecs[0], tvecs[0], 0.1
            )
...
class ThirdPersonCameraCalibrator(CharucoDetector):
    ...
    def _calculate_gripper_to_base(
        ...
        eval_results = self.calculate_target_to_cam(
            eval_readings, train=False
        )
        ...
        R_gripper2target = R.from_euler(
            "xyz", gripper2target[3:]
        ).as_matrix()
        ...
        for i in range(len(eval_R_target2cam)):
            ...
            t_gripper2cam = (
                eval_R_target2cam[i] @ t_gripper2target
                + eval_t_target2cam[i]
            )
        ...
        # Return Poses #
        eulers = np.array(
            [R.from_matrix(rmat).as_euler("xyz") for rmat in rmats]
        )
    ...
    def is_calibration_accurate(self, cam_id):
        ...
        ind = np.random.choice(
            len(readings), size=len(readings), replace=False
        )
        ...
        lin_error = (
            np.linalg.norm(pose_error[:, :3], axis=0) ** 2
            / pose_error.shape[0]
        )
        rot_error = (
            np.linalg.norm(pose_error[:, 3:6], axis=0) ** 2
            / pose_error.shape[0]
        )
...
class HandCameraCalibrator(CharucoDetector):
    ...
    def _calculate_gripper_to_base(
        ...
        for i in range(len(eval_R_target2cam)):
            ...
            t_base2cam = (
                eval_R_target2cam[i] @ t_base2target + eval_t_target2cam[i]
            )
            ...
            R_gripper2base = (
                R.from_matrix(R_base2gripper).inv().as_matrix()
            )
